<template>
  <div class="docs">
    ...
    <!-- HEADER -->
    <alt-header page="docs"></alt-header>

    <!-- Docs Page -->
    <section class="docs-sec">
      <i class="fas fa-bars showBar" @click="showBar"></i>
      <i class="fas fa-times-circle closeBar" @click="closeBar"></i>
      <div class="container-fluid">
        <div class="row">
          <div class="col-lg-3 col-md-4 col-12 docs-show-sidebar pr-0">
            <div class="bar">
              <ul>
                <li v-for="category in categories" :key="category.name">
                  <h4 @click="toggleList($event)">{{ category.name }} 
                    <i class="fas fa-caret-right showList"></i> 
                    <i class="fas fa-caret-down hideList"></i>
                  </h4> 
                  <ul>
                    <li v-if="!category.docs[0]">
                      There Is No Documents Yet
                    </li>
                    <li v-else v-for="doc in category.docs" :key="doc.title" :id="doc.title" @click="showDoc($event)">
                      {{ doc.title }}
                    </li>
                  </ul>
                </li>
          ...
          <div class="col-lg-9 col-md-8 col-12">
            <div class="docs-show-content" id="showContent">
              <h2>Installation</h2>
              <p>Use the package manager npm to install Firework.js.</p>
              <pre><code class="language-shell">
npm install --prefix ./ firework.js
</code>
</pre>
              <p>
                you will find firework.js library files in your folder at path
                <code class="language-bash"
                  >project-folder/node_modules/firework.js</code
                >or include it in your HTML files body tag : Normal Version
              </p>
              <pre><code class="language-html">
&lt;script src="https://cdn.jsdelivr.net/npm/firework.js@1.0.7/firework-dev.js"&gt;&lt;/script&gt;
</code>
</pre>
              <p>
                Since most of the websites have a global layout for whole of the
                application, you might want to start modifying index.html file
...
.docs-show-sidebar {
  ...
  @include sm {
    border: 0;
    margin: 0;
    background: rgba($color: #3F3D56, $alpha: 0.96);
    color: #fff;
...
export default {
  ...
  methods: {
    showBar(){
      $(".docs-show-sidebar").css("transform", "translateX(0)");
      $(".showBar").attr("style", "display: none!important");
      $(".closeBar").attr("style", "display: block!important");
    },
    closeBar(){
      $(".docs-show-sidebar").css("transform", "translateX(-100%)");
      $(".closeBar").attr("style", "display: none!important");
      $(".showBar").attr("style", "display: block!important");
    },
    toggleList(e){
      if(e.target.tagName == "H4"){
        var UL = e.target.nextElementSibling;
        var showI = e.target.children[0];
        var hideI = e.target.children[1];
        var ulHeight = UL.style.height;
        if(ulHeight == 0 || ulHeight == '0px'){
          UL.style.height = 'auto';
          showI.style.display = 'none';
          hideI.style.display = 'inline';
        } else {
          UL.style.height = '0px';
        ...
        var UL = e.path[1].nextElementSibling;
        var showI = e.path[1].children[0];
        var hideI = e.path[1].children[1];
        var ulHeight = UL.style.height;
        if(ulHeight == 0 || ulHeight == '0px'){
          UL.style.height = 'auto';
          showI.style.display = 'none';
      ...
      this.categories.forEach(cat => {
        cat.docs.forEach(doc => {
          if (doc.title == title) {
            ...
            $(".docs-show-content").html(html);
            location.href = "#showContent";
        ...
        Data.forEach(data => {
          ...
          db.get(`documents/${data.val().catName}`, 'child', 'perority').then(Data2 => {
            if (Data2) {
              ...
              $(".bar li:first-of-type ul li:first-of-type").click().addClass("active");
