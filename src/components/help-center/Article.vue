<template>
    ...
    <div class="article">
      <div class="container">
        <div class="row">
          <!-- Go Back Button Slot -->
          <div class="col-12">
            <router-link to="/help" tag="button" class="goback"><i class="fas fa-arrow-circle-left"></i> Back To Articles</router-link>
          </div>

          <!-- Article Content -->
          <div class="col-lg-9 col-md-8 col-12 content" v-html="article.content">
            <h2>What is the basic starter?</h2>
            <p>
              It is a blank directory to make a copy of and start a new project
              with. It includes the required JS and CSS files from TheSaaS as well
              as icon fonts, PHP files and some images.
              <br />
              <br />
              This starter suites for simple websites which doesn't require any
              customization to TheSaaS code and writing few additional CSS and JS
              codes. Simply make a copy and start your project, without npm and
              gulp tasks.
            </p>
            ...
            <p>
              ...
              This starter suites for simple websites which doesn't require any
              customization to TheSaaS code and writing few additional CSS and JS
              codes. Simply make a copy and start your project,<code
                >../assets/images/file.svg</code
              >
  ...
  &lt;script src="https://cdn.jsdelivr.net/npm/firework.js@1.0.7/firework-dev.js"&gt;&lt;/script&gt;
          ...
          <div class="col-lg-3 col-md-4 d-md-block d-none sidebar">
            ...
            <ul>
              <li v-if="!related[0]">
                <img src="../../assets/images/error.svg" /> There is No Related Articles
              </li>
              <router-link v-else tag="li" v-for="relate in related" :key="relate.title" :id="relate.title" :to="'/help/' + relate.title" onclick="location.reload()">
                <img src="../../assets/images/file.svg" /> {{ relate.title }}
              </router-link>
            </ul>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      articleToShow: this.$route.params.id,
      article: {},
      related: []
    };
  },
  created() {
    var title = this.articleToShow;
    document.title = title;
    $(".loader").show();
    db.get("articles").then(AllData => {
      AllData.forEach(Data => {
        db.get(`articles/${Data.val().catName}`).then(data => {
          if (data) {
            data.forEach(data2 => {
              if (typeof data2.val() == "object") {
                if (data2.val().title == title) {
                  this.article = data2.val();
                  db.get(`articles/${data2.val().category}`).then(Data => {
                      if (Data) {
                        Data.forEach(data => {
                          if (typeof data.val() == "object") {
                            if (data.val().title != title) {
                              this.related.push(data.val());
                              $(".loader").hide();
                            } else {
                              $(".loader").hide();
                            }
